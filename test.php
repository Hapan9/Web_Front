...
function OnLoadPage(){
    //$_SESSION['attempt_ID']
    $test_id = json_decode(file_get_contents('http://18.223.196.57:84/api/Authorization/TestAttempt/' . $_SESSION['attempt_ID']))->testId;
    
    $questions = json_decode(file_get_contents('http://18.223.196.57:84/api/Process/MixQuestions?testId=' . $test_id));
            ...
            <thead>
                <tr>
                    <th scope="col" colspan="2">
                        <h3>';
        $string_to_return .= json_decode(file_get_contents('http://18.223.196.57:84/api/Crud/Question/' . $questions[$i]->id))->text;
        ...
        for($j = 0; $j<count($list_of_variants); $j++){
            $string_to_return .= '
                <tr>
                    ...
                    <td>';
            $string_to_return .=  json_decode(file_get_contents('http://18.223.196.57:84/api/Crud/Answer/' . $list_of_variants[$j]))->text;
            $string_to_return .= '
                    </td>
                </tr>';
        }
...
if(isset($_POST['endTest'])){
    ...
    for($i = 0; $i<count($questions); $i++){
         ...
         for($j = 0; $j<=count($list_of_variants); $j++){
             ...
             if(json_decode(file_get_contents('http://18.223.196.57:84/api/Crud/Answer/' . $list_of_variants[$j]))->isCorrect != isset($_POST[$name_of_checkbox])){
                 break;
             }
         }
    }
    
    $data = json_decode(file_get_contents("http://18.223.196.57:84/api/Authorization/TestAttempt/". $_SESSION['attempt_ID']));
    $data->testGrade = $mark;
